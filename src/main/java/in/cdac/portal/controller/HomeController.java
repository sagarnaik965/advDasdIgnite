...
import in.cdac.portal.service.EmailServiceImpl;
...
//	ReadIgniteService readSer;
	...
	@Autowired
	EmailServiceImpl emailServiceIm;
	...
	public String QueryEmailData(@ModelAttribute("Query Data") QueryForEmail querydata, Model model) {
		try {
			if (querydata.getMobieNo().trim().length() == 10) {
				
			} else {
				model.addAttribute("OnFail", "Enter Correct Mobile No . . ");
				return "SendEmail";
			}
			emailServiceIm.sendMail(querydata, Boolean.parseBoolean(env.getProperty("bcc")));
			model.addAttribute("OnSucces", "Your Query Has been saved succesfully");
			return "ResponsePage";
		} catch (MailException mailException) {
			model.addAttribute("OnFail", "Please Try again . . ");
			return "SendEmail";
